...
\item the overall object velocity ($\sqrt{obj_{vx}^{cur} * obj_{vx}^{cur} + obj_{vy}^{cur} * obj_{vy}^{cur}}$), and
\item the discrepancy in predicted x-position ($obj_x^{past} + obj_{vx}^{past}$) and y-position ($obj_y^{past} + obj_{vy}^{past}$).
\end{enumerate}
